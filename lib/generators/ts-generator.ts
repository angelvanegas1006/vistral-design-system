...
export function generateTypeScript(tokens: DesignTokens): string {
  ...
  if (tokens.typography.fontFamily) {
    const fontFamilyKeys = Object.keys(tokens.typography.fontFamily).map((k) => `"${k}"`).join(" | ")
  ...
  if (tokens.typography.fontSize) {
    const fontSizeKeys = Object.keys(tokens.typography.fontSize).map((k) => `"${k}"`).join(" | ")
  ...
  // Spacing type
  const spacingKeys = Object.keys(tokens.spacing).map((k) => `"${k}"`).join(" | ")
  lines.push(`  spacing: Record<${spacingKeys}, string>`)

  // Radius type
  const radiusKeys = Object.keys(tokens.radius).map((k) => `"${k}"`).join(" | ")
  lines.push(`  radius: Record<${radiusKeys}, string>`)

  // Shadows type (optional)
  if (tokens.shadows) {
    const shadowKeys = Object.keys(tokens.shadows).map((k) => `"${k}"`).join(" | ")
  ...
  if (tokens.breakpoints) {
    const breakpointKeys = Object.keys(tokens.breakpoints).map((k) => `"${k}"`).join(" | ")
