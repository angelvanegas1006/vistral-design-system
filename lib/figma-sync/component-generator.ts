  ...
  if (component.variants && Object.keys(component.variants).length > 0) {
    for (const [variantName, options] of Object.entries(component.variants)) {
      ...
      if (propName === "variant") {
        ...
        let defaultOption = normalizedOptions.find(o => o === "primary" || o === "default") || normalizedOptions[0]
        defaults.push(`      ${propName}: "${defaultOption}",`)
      } else if (propName === "size") {
        // Prefer "md" or "default" over "lg" as default
        let defaultOption = normalizedOptions.find(o => o === "md" || o === "default") || normalizedOptions[0]
        defaults.push(`      ${propName}: "${defaultOption}",`)
      }
    }
  } else {
...
export function generateComponent(component: ComponentInfo): string {
  ...
  const elementRefType = isButton ? "HTMLButtonElement" : elementType === "input" ? "HTMLInputElement" : elementType === "a" ? "HTMLAnchorElement" : "HTMLDivElement"
  const elementPropsType = isButton ? "React.ButtonHTMLAttributes<HTMLButtonElement>" : elementType === "input" ? "React.InputHTMLAttributes<HTMLInputElement>" : elementType === "a" ? "React.AnchorHTMLAttributes<HTMLAnchorElement>" : "React.HTMLAttributes<HTMLDivElement>"
  ...
  const template = componentTemplate
    ...
    .replace(/{{BASE_CLASSES}}/g, isButton ? "inline-flex items-center justify-center rounded-md font-medium transition-colors focus-visible:outline-none focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50" : "inline-flex items-center justify-center")
...
export function generateStory(component: ComponentInfo): string {
      ...
      for (let i = 0; i < variantOptions.length; i++) {
        ...
        if (!usedStoryNames.has(storyName)) {
          ...
          if (sizeProp) {
            ...
            variantStories.push(`    ${toPropName(sizeProp)}: "${normalizedSizes[1] || normalizedSizes[0]}",`)
          }
          variantStories.push(`  },`)
          variantStories.push(`}`)
          variantStories.push("")
  ...
  const variantProp = component.variants ? Object.keys(component.variants).find(k => toPropName(k) === "variant") : null
  const sizeProp = component.variants ? Object.keys(component.variants).find(k => toPropName(k) === "size") : null
  const defaultVariant = variantProp 
    ? normalizeVariantOption(component.variants[variantProp].find(o => o.toLowerCase().includes("primary")) || component.variants[variantProp][0])
    : "default"
  const defaultSize = sizeProp
    ? normalizeVariantOption(component.variants[sizeProp].find(o => o.toLowerCase() === "md" || o.toLowerCase() === "default") || component.variants[sizeProp][0])
    : "default"
