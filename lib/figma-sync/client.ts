...
export async function getFigmaFile(fileId: string = figmaConfig.fileId): Promise<FigmaFile> {
  ...
  try {
    ...
    throw new Error(
      `Failed to fetch Figma file ${fileId}: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
export async function getFigmaFileNodes(
  ...
  for (let i = 0; i < nodeIds.length; i += batchSize) {
    ...
    try {
      const data = await fetchWithRetry<{ nodes: Record<string, any> }>(
        url,
        {},
        {
          retries: 5,
      ...
      console.warn(
        `⚠️  Failed to fetch batch of ${batch.length} nodes:`,
...
export async function getFigmaFileStyles(
  ...
  try {
    ...
    throw new Error(
      `Failed to fetch Figma styles: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
export async function getFigmaStyleValues(
  ...
  for (let i = 0; i < styleNodeIds.length; i += batchSize) {
    ...
    try {
      const data = await fetchWithRetry<{ nodes: Record<string, { document: any }> }>(
        url,
        {},
        {
          retries: 5,
      ...
      console.warn(
        `⚠️  Failed to fetch style values batch:`,
        error instanceof Error ? error.message : String(error)
      )
    }
  }
