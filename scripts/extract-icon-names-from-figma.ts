...
async function main() {
  ...
  try {
    ...
    for (const categoryName of ICON_CATEGORIES) {
      const categoryNodes = findNodes(document, node => {
      ...
      for (const categoryNode of categoryNodes) {
        ...
        const icons = extractIconsFromCategory(
          categoryNode,
    ...
    const componentSets = findNodes(
      ...
      node =>
        ...
        (node.name.toLowerCase().includes('icon') || node.name.toLowerCase().includes('symbol'))
    )
    ...
    // Remove duplicates
    const uniqueIcons = Array.from(new Map(allIcons.map(icon => [icon.nodeId, icon])).values())
