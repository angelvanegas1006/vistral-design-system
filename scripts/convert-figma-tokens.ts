...
const primitives = JSON.parse(
  fs.readFileSync(path.join(tokensDir, '1. Primitives.Light.tokens.json'), 'utf-8')
)
const semantic = JSON.parse(
  fs.readFileSync(path.join(tokensDir, '2. Semantic.Light.tokens.json'), 'utf-8')
)
const components = JSON.parse(
  fs.readFileSync(path.join(tokensDir, '3. Components .Light.tokens.json'), 'utf-8')
)
const textStyles = JSON.parse(
  fs.readFileSync(path.join(tokensDir, 'text.styles.tokens.json'), 'utf-8')
)
const effectStyles = JSON.parse(
  fs.readFileSync(path.join(tokensDir, 'effect.styles.tokens.json'), 'utf-8')
)
...
function convertColors() {
  ...
  if (semColor?.bg) {
    ...
    if (semColor.bg['success-alt'])
      semanticColors['success-bg-alt'] = getValue(semColor.bg['success-alt'])
    if (semColor.bg.warning) semanticColors['warning-bg'] = getValue(semColor.bg.warning)
    if (semColor.bg['warning-alt'])
      semanticColors['warning-bg-alt'] = getValue(semColor.bg['warning-alt'])
    if (semColor.bg.error) semanticColors['error-bg'] = getValue(semColor.bg.error)
    if (semColor.bg['error-alt'])
      semanticColors['error-bg-alt'] = getValue(semColor.bg['error-alt'])
    if (semColor.bg.primary) semanticColors['bg-primary'] = getValue(semColor.bg.primary)
    if (semColor.bg['primary-alt'])
      semanticColors['bg-primary-alt'] = getValue(semColor.bg['primary-alt'])
  ...
  if (semColor?.fg) {
    ...
    if (semColor.fg['brand-hover'])
      semanticColors['fg-brand-hover'] = getValue(semColor.fg['brand-hover'])
  }
  if (semColor?.border) {
    if (semColor.border.default)
      semanticColors['border-default'] = getValue(semColor.border.default)
    if (semColor.border.elevated)
      semanticColors['border-elevated'] = getValue(semColor.border.elevated)
    ...
    if (semColor.border.success)
      semanticColors['border-success'] = getValue(semColor.border.success)
    if (semColor.border.warning)
      semanticColors['border-warning'] = getValue(semColor.border.warning)
  }
...
console.log(
  `   Colors: ${Object.keys(finalTokens.colors.primary).length} primary, ${Object.keys(finalTokens.colors.semantic).length} semantic, ${Object.keys(finalTokens.colors.component).length} component`
)
