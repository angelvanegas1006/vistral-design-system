...
async function syncTokens() {
  ...
  try {
    ...
    if (tokens.documentation) {
      ...
      console.log(`📚 Documentation extracted from ${sections.length} sections: ${sections.join(", ")}`)
    }
  } catch (error) {
...
async function listComponents() {
  ...
  try {
    ...
    const allMainComponents = Array.from(new Set([...componentSets, ...Array.from(mainComponentNames)]))
      .sort()
    ...
    console.log(`📊 Found ${filtered.length} main components${searchTerm ? ` matching "${process.argv[4]}"` : " (showing first 100)"}:\n`)
    
    filtered.forEach(name => {
      // Count how many variants/components belong to this main component
      const variants = allComponentNames.filter(n => 
        n.split(/[=,]/)[0].split("/")[0].trim() === name
      )
  ...
  try {
    ...
    if (componentFilter) {
      ...
      filterFn = (component: any, name: string) => {
        ...
        const isVariantMatch = nameLower.startsWith(filterName + "=") || 
                               nameLower.match(new RegExp(`^${filterName}=`)) ||
                               (nameLower.includes("type=") && nameLower.includes(filterName) && nameLower.includes("variant="))
    ...
    console.error("\n❌ Component sync failed:", error instanceof Error ? error.message : String(error))
    process.exit(1)
  }
}
...
async function main() {
  switch (command) {
    ...
    default:
      ...
      console.log("\nUsage:")
      console.log("  npm run figma:sync:tokens                    # Sync all tokens from FOUNDATION")
